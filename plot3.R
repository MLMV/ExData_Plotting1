...
# Project: Coursera Data Science Specialization
# Task: Exploratory Data Analysis Week 1 Assignment
# Date: March 20, 2016
...
# For each plot you should
      ...
      # constructs the corresponding plot, i.e. code in plot1.R constructs the
      # plot1.png plot. Your code file should include code for reading the data
      # so that the plot can be fully reproduced. You must also include the code
...
# The four plots that you will need to construct are shown at
...
# When loading the dataset into R, please consider the following:
      ...
      # 3. You may find it useful to convert the Date and Time variables to
...
# Plan --------------------------------------------------------------------
      ...
      # Calculate memory size 2,075,259 rows * 9 columns * 8 bytes = 144,000,000
      # bytes. 144,000,000 bytes / 2^20 = 137Mb.
      # Load data
      # 
...
# replace this to set your own working folder to where your data is
setwd("~/R/Coursera/4. Exploratory Data Analysis/C4W1_Assignment")
...
# Subset
subdata <- subset(rawdata, Date == "2007-02-01" | Date == "2007-02-02")

# New field for date & time
subdata$DateTime <- as.POSIXlt(
      paste(
            subdata$Date,
            subdata$Time
            ),
...
# Bin raw data to free up memory
remove(rawdata)

# Transform to long format on Submetering in order to make the plot
# 
# This requires date->char->date conversion as melt refuses to work on dates
subdata2 <- subdata

# date to char
subdata2$DateTime <- as.character(subdata2$DateTime)
...
# char to date
transdata$DateTime <- as.POSIXlt(
      transdata$DateTime,
      "%Y-%m-%d %H:%M:%S",
      tz = "UTC"
      )

# make neat column names
transdata <- rename(transdata, watthours = value, metering = variable)

# make subsets for plotting the lines
sub1 <- subset(transdata, metering == "Sub_metering_1")
sub2 <- subset(transdata, metering == "Sub_metering_2")
sub3 <- subset(transdata, metering == "Sub_metering_3")
